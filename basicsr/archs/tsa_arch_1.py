...
#         self.feature_extraction = make_layer(ResidualBlockNoBN, 3, num_feat=num_feat) #   ResidualBlockRCB
...
#                 feat_prop = self.LTAM(lr_curr_feat, index_feat_buffer_s1, feat_prop, location_update) # memory_buffer,
...
class TSA(nn.Module):
    def __init__(self,num_in_ch=3,num_out_ch=3,num_feat=64,num_frame=15):
        ...
        self.resblocks = make_layer(ResidualBlockNoBN, 13, num_feat=num_feat)
        # align  Trajectory-Aware Alignment
        self.LTAM_noTrj = LTAM_noTrj(stride = self.stride, k_num = self.k_num)
        # upsample
        self.upconv1 = nn.Conv2d(num_feat, 48, 3, 1, 1)
        self.pixel_shuffle = nn.PixelShuffle(4)
    ...
    def forward(self, lrs, gts=None):
        ...
        for i in range(0, t):
            ...
            if i > 0:  # no warping required for the first timestep
                ...
                # index_feat_buffer_s1 = torch.stack(index_feat_buffers_s1, dim=1)
                if self.k_num <= i:
                    corr_index_list = outputs[i-self.k_num:i]
                else:
                    # index_feat_buffer_s1 = outputs[i-1]
                    [corr_index_list.append(outputs[i-1])  for i in range(self.k_num)][0]

                # index_feat_buffer_s1 = outputs[i-self.k_num:i]
                feat_prop = self.LTAM_noTrj(lr_curr_feat, corr_index_list, feat_prop) # memory_buffer,
                # location_update = torch.cat([location_update,torch.stack([grid_x,grid_y],dim=0).type_as(lrs).expand(n,-1,-1,-1)],dim=1)
                # if gts != None:
                #     if gt_flows_forward is not None:
                #         gt_flow = gt_flows_forward[:, i - 1, :, :, :]
...
class LTAM_noTrj(nn.Module):
    ...
    def forward(self, curr_feat,  index_feat_set_s1, anchor_feat):
        ...
        for ii in range(self.k_num-1,-1,-1):
            corr_soft = index_feat_set_s1[ii]
            ...
            out_list.append(self.fusion0(corr_soft*anchor_feat))

        ##### Mamba Aggregation
        input_mamba = torch.cat(out_list,0)
        out = self.S_Mamba(input_mamba)

        output = self.fusion2(out.reshape(n, -1, h, w))
        return output
...
class LTAM(nn.Module):
    ...
    def forward(self, curr_feat,  index_feat_set_s1, anchor_feat, location_feat):
        """Compute the long-range trajectory-aware Mamba.
        Args:
            anchor_feat (tensor): Input feature with shape (n, c, h, w)
            location_feat (tensor): Input location map with shape (n, 2*t, h//4, w//4)
...
class Shfited_Mamba_Block(nn.Module):
    ...
    def forward(self, input_x):
        ...
        # x = (x_shift1 + x_shift3) * self.skip_scale2 + self.conv_blk(self.ln_2(x_shift1 + x_shift3).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
