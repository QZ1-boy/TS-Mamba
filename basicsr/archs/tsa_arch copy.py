    ...
    def forward(self, lrs, hidden_states=None, return_hs=False):
        ...
        for i in range(0, t):
            ...
            if i > 0:  # no warping required for the first timestep
                ...
                feat_prop = self.defor_att(lr_curr_feat,feat_prop,out_feat,flow)
                ...
                feat_prop = self.LTAM(lr_curr_feat, index_feat_buffer_s1,feat_prop,sparse_feat_buffer_s1,location_update)  #  sparse_feat_buffer_s2,
            ...
            if i in keyframe_idx_forward:
                # feature tokenization *2
                # bs * c * h * w --> # bs * (c*2*2) * (h//2*w//2)
                # sparse_feat_prop_s0 = F.unfold(feat_prop, kernel_size=(int(0.5*self.stride),int(0.5*self.stride)), padding=0, stride=int(0.5*self.stride))
                # # bs * (c*2*2) * (h//2*w//2) -->  bs * c * (h*4) * (w*4)
                # # print('sparse_feat_prop_s0',sparse_feat_prop_s0.shape)
                # sparse_feat_prop_s0 = F.fold(sparse_feat_prop_s0, output_size=(int(2*h),int(2*w)), kernel_size=(int(0.5*self.stride),int(0.5*self.stride)), padding=0, stride=int(self.stride))
                # # bs * c * (h*4) * (w*4) -->  bs * c * h * w
                # sparse_feat_prop_s0 = F.interpolate(sparse_feat_prop_s0, size=(h,w), mode='bilinear', align_corners=False)
                # # sparse_feat_prop_s2 = F.adaptive_avg_pool2d(sparse_feat_prop_s2,(h,w))
                # # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)
                # sparse_feat_prop_s0 = F.unfold(sparse_feat_prop_s0, kernel_size=(self.stride,self.stride), padding=0, stride=self.stride)
                # # bs * (c*4*4) * (h//4*w//4) -->  bs * (c*4*4) * h//4 * w//4
                # sparse_feat_prop_s0 = F.fold(sparse_feat_prop_s0, output_size=(h//self.stride,w//self.stride), kernel_size=(1,1), padding=0, stride=1)
                # sparse_feat_buffers_s0.append(sparse_feat_prop_s0)
                ...
                # sparse_feat_prop_s2 = F.interpolate(sparse_feat_prop_s2, size=(h,w), mode='bilinear', align_corners=False)
                # # sparse_feat_prop_s2 = F.adaptive_avg_pool2d(sparse_feat_prop_s2,(h,w))
                # # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)
                ...
                # sparse_feat_prop_s3 = F.interpolate(sparse_feat_prop_s3, size=(h,w), mode='bilinear', align_corners=False)
                # # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)
...
class LTAM(nn.Module):
    def __init__(self, stride=4, k_num=3):
        ...
        #                                 n_heads=num_heads, n_points=num_points,
        #                                 max_residue_magnitude=max_residue_magnitude)
    def forward(self, curr_feat,  index_feat_set_s1 , anchor_feat, sparse_feat_set_s1, location_feat):
        ...
        # output_s0 = F.grid_sample(sparse_feat_set_s0.contiguous().view(-1,(c*self.stride*self.stride),(h//self.stride),(w//self.stride)),grid_flow.contiguous().view(-1,(h//self.stride),(w//self.stride),2),mode='nearest',padding_mode='zeros',align_corners=True) # (nt) * (c*4*4) * (h//4) * (w//4)
        ...
        if self.k_num <= t:
            ...
            for i in range(self.k_num):
                #### Top-2
                corr_soft_list.append(corr_soft_k[:,:,i])
                corr_index_list.append(corr_index_k[:,:,i])
        else:
            corr_soft, corr_index = torch.max(matrix_index, dim=2)  # n * (h//4*w//4)
            for i in range(self.k_num):
                corr_soft_list.append(corr_soft)
        ...
        for ii in range(self.k_num-1,-1,-1):
            ...
            out_list.append(output_s1_f)


        output_s1_f = [out + out_list[i] for i in range(len(out_list))][0]
        output_agg = self.fusion(torch.cat([output_s1_f, feat_curr0, anchor_feat], dim=1))
        output_agg = self.defor_att[ii](output_agg, feat_curr0, corr_soft, None)
...
class LTAM_bk(nn.Module):
    def __init__(self, stride=4):
        super().__init__()
        self.stride = stride
        # self.fusion = nn.Conv2d(4 * 64, 64, 1, 1, 0, bias=True)
        self.RCB = RCB(64, reduction=2)
        num_heads = 4
        num_points = 6
        max_residue_magnitude = 10
        num_levels = 1
        # self.defor_att = DeformAttnAlign(d_model=64,
        #                                 n_levels=num_levels,
        #                                 n_heads=num_heads,
        #                                 n_points=num_points,
        #                                 max_residue_magnitude=max_residue_magnitude)
    def forward(self, curr_feat, sparse_feat_set_s0, index_feat_set_s1 , anchor_feat, sparse_feat_set_s1 , sparse_feat_set_s3, location_feat):
        """Compute the long-range trajectory-aware attention.  sparse_feat_set_s2,
        Args:
            anchor_feat (tensor): Input feature with shape (n, c, h, w)
            sparse_feat_set_s0 (tensor): Input tokens with shape (n, t, c*4*4, h//4, w//4)
        ...
        # n * (h//4*w//4) --> n * (c*4*4) * (h//4*w//4)
        # corr_soft00, corr_index00 = torch.topk(matrix_index, k=1, dim=2)# n * (h//4*w//4)
        # print(corr_soft==corr_soft00, corr_index==corr_index00, matrix_index.shape)
        k = 3
        # print(t,k)
        corr_soft_list = []
        corr_index_list = []
        if k <= t:
            corr_soft_k, corr_index_k = torch.topk(matrix_index, k=k, dim=2)# n * (h//4*w//4)
            for i in range(k):
                #### Top-2
                # print('corr_soft_i',corr_soft_k.shape)
                corr_soft_list.append(corr_soft_k[:,:,i])
                corr_index_list.append(corr_index_k[:,:,i])
        else:
            corr_soft, corr_index = torch.max(matrix_index, dim=2)# n * (h//4*w//4)
            for i in range(k):
                # print('corr_soft',corr_soft.shape)
                corr_soft_list.append(corr_soft)
                corr_index_list.append(corr_index)
        ...
        for ii in range(k-1,-1,-1):
            ...
            # # Aggr
            # # n * t * (c*4*4) * h//4 * w//4 --> nt * (c*4*4) * h//4 * w//4
            # output_s0 = output_s0.contiguous().view(n*t,(c*self.stride*self.stride),(h//self.stride),(w//self.stride))
            # # nt * (c*4*4) * h//4 * w//4 --> n * t * (c*4*4) * (h//4*w//4)
            # output_s0 = F.unfold(output_s0, kernel_size=(1, 1), padding=0, stride=1).view(n,-1,feat_len,feat_num)
            # # n * t * (c*4*4) * (h//4*w//4) --> n * 1 * (c*4*4) * (h//4*w//4)
            # output_s0 = torch.gather(output_s0.contiguous().view(n,t,feat_len,feat_num), 1, corr_index.view(n,1,1,feat_num).expand(-1,-1,feat_len,-1))# n * 1 * (c*4*4) * (h//4*w//4)
            # # n * 1 * (c*4*4) * (h//4*w//4)  --> n * (c*4*4) * (h//4*w//4)
            # output_s0 = output_s0.squeeze(1)
            # # n * (c*4*4) * (h//4*w//4) --> n * c * h * w
            # output_s0 = F.fold(output_s0, output_size=(h,w), kernel_size=(self.stride,self.stride), padding=0, stride=self.stride)
            # output_agg0 = self.defor_att(output_s0, feat_curr0, corr_soft, None)
            # output_agg0 = self.RCB(output_s0,feat_curr0)


            # Aggr
            # n * t * (c*4*4) * h//4 * w//4 --> nt * (c*4*4) * h//4 * w//4
            # output_s1 = (output_s1).contiguous().view(n*t,(c*self.stride*self.stride),(h//self.stride),(w//self.stride))
            # # output_s1_ = output_s1 +
            # # nt * (c*4*4) * h//4 * w//4 --> n * t * (c*4*4) * (h//4*w//4)
            ...
            # # n * (c*4*4) * (h//4*w//4) --> n * c * h * w
            # output_s1 = F.fold(output_s1, output_size=(h,w), kernel_size=(self.stride,self.stride), padding=0, stride=self.stride)
            # output_agg1 = self.defor_att(output_s1, feat_curr0, corr_soft, None)
            # output_agg1 = self.RCB(output_agg0+output_s1,feat_curr0)

            # Aggr
            # n * t * (c*4*4) * h//4 * w//4 --> nt * (c*4*4) * h//4 * w//4
            # output_s2 = (output_s2).contiguous().view(n*t,(c*self.stride*self.stride),(h//self.stride),(w//self.stride))
            # # output_s2_ = output_s1_ + output_s2
            # # nt * (c*4*4) * h//4 * w//4 --> n * t * (c*4*4) * (h//4*w//4)
            ...
            # Aggr
            # # n * t * (c*4*4) * h//4 * w//4 --> nt * (c*4*4) * h//4 * w//4
            # output_s3_v = (output_s3).contiguous().view(n*t,(c*self.stride*self.stride),(h//self.stride),(w//self.stride))
            # # nt * (c*4*4) * h//4 * w//4 --> n * t * (c*4*4) * (h//4*w//4)
            # output_s3_u = F.unfold(output_s3_v, kernel_size=(1, 1), padding=0, stride=1).view(n,-1,feat_len,feat_num)
            # # n * t * (c*4*4) * (h//4*w//4) --> n * 1 * (c*4*4) * (h//4*w//4)
            # output_s3_g = torch.gather(output_s3_u.contiguous().view(n,t,feat_len,feat_num), 1, corr_index.view(n,1,1,feat_num).expand(-1,-1,feat_len,-1))# n * 1 * (c*4*4) * (h//4*w//4)
            # # n * 1 * (c*4*4) * (h//4*w//4) --> n * (c*4*4) * (h//4*w//4)
            # output_s3_s = output_s3_g.squeeze(1)
            # # n * (c*4*4) * (h//4*w//4) --> n * c * h * w
            # output_s3_f = F.fold(output_s3_s, output_size=(h,w), kernel_size=(self.stride,self.stride), padding=0, stride=self.stride)
            output_agg3 = self.RCB(output_s3+output_agg1,feat_curr0)

            # output_agg3 = self.defor_att(output_s3+output_s2, feat_curr0, corr_soft, None)
            # output_agg = torch.cat([output_s0,output_s1,output_s2,output_s3], dim=1)
            # output_agg = self.fusion(output_agg)
            # output_agg = self.defor_att(output_agg, feat_curr0, corr_soft, None)
            # output_agg = self.RCB(output_agg,feat_curr0)
            # output_agg = self.defor_att(output_sum, feat_curr0, corr_soft, None)
            # output_agg = output_agg * corr_soft
            out = out + output_s3_f * corr_soft + anchor_feat
