...
class ResidualBlockRCB(nn.Module):
    """Residual block with RCB.
    Args:
        num_feat (int): Channel number of intermediate features.
            Default: 64.
        res_scale (float): Residual scale. Default: 1.
    ...
    def __init__(self, num_feat=64, res_scale=1, pytorch_init=False):
        super(ResidualBlockRCB, self).__init__()
        self.res_scale = res_scale
        self.conv1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(num_feat, num_feat, 3, 1, 1, bias=True)
        self.relu = nn.ReLU(inplace=True)
        # self.RCB = RCB(n_feat=num_feat,reduction=2)
        self.gcnet = ContextBlock(n_feat=num_feat, bias=False)
    ...
    def forward(self, x):
        identity = x
        out = self.gcnet(self.conv2(self.relu(self.conv1(x))))
        return identity + out * self.res_scale
