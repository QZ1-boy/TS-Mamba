...
class TSA(nn.Module):
    def __init__(self,num_in_ch=3,num_out_ch=3,num_feat=64,num_frame=15,center_frame_idx=None,hr_in=False):
        super().__init__()
        ...
        self.resblocks = make_layer(ResidualBlockRCB, 13, num_feat=num_feat)
    ...
    def forward(self, lrs, hidden_states=None, return_hs=False):
        ...
        for i in range(0, t):
            ...
            if i > 0:  # no warping required for the first timestep
                ...
                # n , 2t , h , w
                location_update = flow_warp(location_update, flow.permute(0, 2, 3, 1),padding_mode='border',interpolation="nearest")
                ...
                feat_prop = self.LTAM(lr_curr_feat, index_feat_buffer_s1, feat_prop, memory_buffer, location_update)
            ...
            # bs * c * h * w --> # bs * (c*4*4) * (h//4*w//4)   ######## Why out_feat?  #########
...
class LTAM(nn.Module):
    def __init__(self, stride=4, k_num=2):
        super().__init__()
        self.stride = stride
        self.k_num = k_num
        self.fusion = nn.Conv2d(3 * 64, 64, 1, 1, 0, bias=True)
        self.fusion1 = nn.Conv2d((self.k_num+1) * 64, 64, 1, 1, 0, bias=True)
        # split_list = np.linspace(0.5, 1, num = self.k_num+1) # np.arange(0,0.5,self.k_num+1)
        self.MambaBlock = BiMambaBlock(dim=64,
                                        depth=1,
                                        k_num=self.k_num,
                                        norm_layer=nn.LayerNorm,
                                        downsample=None)

        self.Mambablock = nn.ModuleList([LC_Mamba_Block(
                            hidden_dim=64,
                            norm_layer=nn.LayerNorm,
                            d_state=16,
                            expand=2,
                            window_size = window_sizes[8,8],
                            shift_size = 1*window_sizes[0]//8)
                            ])
    ...
    def forward(self, curr_feat,  index_feat_set_s1, anchor_feat, memory_buffer_set, location_feat):
        ...
        for ii in range(self.k_num-1,-1,-1):
            ...
            # # # n * (c*4*4) * (h//4*w//4) --> n * c * h * w
            # output_s1_f = F.fold(output_s1_s, output_size=(h,w), kernel_size=(self.stride,self.stride), padding=0, stride=self.stride)            ######## 实现 Mamba的token alignment 对齐
            ...
            out_list.append(output_agg)
            ##### Mamba Fusion
        out = self.MambaBlock(out_list)
        output = self.fusion1(out.reshape(n, -1, h, w))
        # print('out',out.shape)
        return output
...
class SS2D(nn.Module):
    ...
    def merge_x(self, x):
        B, C, H, W = x.shape
        B_each = B//(self.k_num+1)
        L = 2 * H * W
        x = x.view(B, -1, L//2).transpose(1, 2)
        x = torch.cat([x[:B//2], x[B//2:]], dim=-1).reshape(B//2, L, C)  ###  1 1 2 2 3 3 4 4
        # x = torch.cat([x[:], x[:]], dim=-1).reshape(B, L, C)
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        # torch.transpose(x, dim0=2, dim1=3) ###  1 3 1 3 2 4 2 4
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y = torch.cat([y[:, :, 0], y[:, :, 1]], 0).view(B, int(self.expand*C), H, W)# .view(B//2, 2*H, 2*W, -1) ## *2
        # y = self.conv2dfuse(y).view(B, H, W, int(self.expand*C))
        y = y.view(B, H, W, int(self.expand*C))
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.ln_1 = norm_layer(hidden_dim)
        self.self_attention = SS2D(d_model=hidden_dim, k_num=k_num, d_state=d_state,expand=mlp_ratio,dropout=attn_drop_rate, **kwargs)
        self.skip_scale= nn.Parameter(torch.ones(hidden_dim))
        self.conv_blk = CAB(hidden_dim)
        self.ln_2 = nn.LayerNorm(hidden_dim)
        self.skip_scale2 = nn.Parameter(torch.ones(hidden_dim))
