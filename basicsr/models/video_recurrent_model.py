...
class VideoRecurrentModel(VideoBaseModel):
    ...
    def test(self):
        ...
        with torch.no_grad():
            # print('self.lq',self.lq.shape) # [1, 41, 3, 144, 180]
            N, T, C, H, W = self.lq.shape  # 示例尺寸
            pad_h = (8 - (H % 8)) % 8  # 对于H，确保是8的倍数
            pad_w = (8 - (W % 8)) % 8  # 对于W，确保是8的倍数
            # print('pad',pad_h,pad_w)
            # self.lq_ = F.pad(self.lq.squeeze(0), (pad_w//2, pad_w-pad_w//2, pad_h//2, pad_h-pad_h//2), "replicate").unsqueeze(0)
            # print('self.lq_',self.lq.shape, self.lq_.shape)

            self.output = self.net_g(self.lq)  # self.lq_
             #print('self.output',self.output.shape)
            # self.output = self.output[:,:,:,2*pad_h:4*H+2*pad_h,2*pad_w:4*W+2*pad_w]
            # print('self.output',self.output.shape)
